...
public class RegionLog extends Table {
	...
	public static Results getLast(@Nonnull final Regions region) {
		return SL.getDB()
		         .dq("select tds,changetype,oldvalue,newvalue from regionlog where regionid=? order by tds desc limit 0,100",
		             region.getId());
	}
	...
	public static void log(final int region,
	                       ...
	                       final int tds) {
		SL.getDB()
		  .d("insert into regionlog(regionid,changetype,oldvalue,newvalue,tds) values(?,?,?,?,?)",
		     region,
		     changetype,
		     oldvalue,
		     newvalue,
