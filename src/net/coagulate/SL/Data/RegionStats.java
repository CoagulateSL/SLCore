...
public class RegionStats extends Table {
	...
	public static void log(final int region,
	                       final int timestamp,
	...
	public static void log(@Nonnull final Regions region,
	                       final int timestamp,
	                       final String statstype,
	                       final float min,
	                       final float max,
	...
	public static Results graphableData(@Nonnull final Regions r,final String stattype,int from,int to,final int x) {
		...
		if (timerange==0) {
			throw new UserInputValidationParseException("No time range covered?");
		}
		
		
		// range is "from" to "to", subtract the from, divide by the total range, scale to size
		return d.dqSlow("select "+"round(?*((timestamp-?)/?)) as x,"+"timestamp,"+"min(statmin) as plotmin,"+
		                "max(statmax) as plotmax,"+"avg(statavg) as plotavg,"+"avg"+"(statsd) as plotsd,"+"samplesize "+
		                "from regionstats "+"where timestamp>=? "+"and timestamp<=? "+"and stattype=? "+
		                "and regionid=? "+"group "+"by x "+"order by timestamp asc",
		                x,
		                from,
		                timerange,
		                from,
	...
	public static void archiveOld() {
		...
		if (rollups>0) {
			log.fine("Completed region stats archiving, batched into "+rollups+" HOURLY samples");
		}
		rollups=0;
		for (final ResultsRow r: d.dq(
				"select floor(timestamp/(60*60*24)) as basetime,regionid,stattype,min(statmin) as newmin,max(statmax) as newmax,avg(statavg) as newavg,avg(statsd) as newsd "+
				"from regionstats where timestamp<? and samplesize='HOURLY' group by basetime,regionid,stattype",
				start-(60*60*24*7*2))) {
			if ((UnixTime.getUnixTime()-start)>30) {
				log.fine("Stopping incomplete archival run due to runtime>30 seconds");
				return;
			...
			final float avg=r.getFloat("newavg");
			final float sd=r.getFloat("newsd");
			basetime=basetime*60*60*
			         24; // we divided above, to split into hourly blocks, but we need a full time reference
		...
		if (rollups>0) {
			log.fine("Completed region stats archiving, batched into "+rollups+" DAILY samples");
		}
	}
