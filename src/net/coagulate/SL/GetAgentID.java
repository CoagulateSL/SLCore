...
public class GetAgentID {
	...
	public static String getAgentID(@Nonnull String name) {
		...
		if (Config.getGrid()!=Config.GRID.SECONDLIFE) {
			throw new UserConfigurationException("Unable to try resolve this name on this grid type");
		}
		if (Config.getSecondLifeAPIKey().isEmpty()) {
			throw new UserConfigurationException("Unable to try resolve this name due to lack of Name Service API Key");
		}
		...
		try {
			...
			switch (responsecode) {
				case 403 -> {
					final SystemRemoteFailureException urfe=
							new SystemRemoteFailureException("SL Name API Rate Limited");
					SL.report("Name API IO Error",urfe,null);
					throw urfe;
				}
				case 405 -> {
					final SystemRemoteFailureException srfe=
							new SystemRemoteFailureException("SL Name API said Malformed Request");
					SL.report("Name API IO Error",srfe,null);
					throw srfe;
				}
				case 500 -> {
					final UserRemoteFailureException error=
							new UserRemoteFailureException("SL Name API service errored");
			...
			if (responsecode==200) {
				rd=new BufferedReader(new InputStreamReader(transmission.getInputStream()));
			} else {
				rd=new BufferedReader(new InputStreamReader(transmission.getErrorStream()));
			}
			String line;
			...
			if (responsecode==404) {
				throw new UserRemoteFailureException("SL Name API - "+json.optString("error","???")+" - "+
				                                     json.optString("message","NoErrorMessage"));
			}
