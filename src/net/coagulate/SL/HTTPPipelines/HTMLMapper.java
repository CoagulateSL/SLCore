...
public class HTMLMapper extends URLMapper<Method> {
	...
	public static synchronized URLMapper<Method> get() {
		if (singleton==null) {
			singleton=new HTMLMapper();
		}
		return singleton;
	}
	
	// ---------- INSTANCE ----------
	@Url(url="/login", authenticate=false)
	// note the url part is irrelevant as this intercepts authentication required URLs.  You can go here too for fun.
	public static void getAuthenticationPage(@Nonnull final State state) {
		...
		table.row()
		     .header("Avatar Name:")
		     .alignCell("right")
		     .data(new InputText("login_username").size(20).autofocus());
		table.row()
		     .header(SL.brandNameUniversal()+" Password:")
		     .alignCell("right")
		     .data(new InputPassword("login_password").size(20));
		if (SL.canIM()) {
			table.row()
			     .data("If you do not enter a password, your avatar will be IMed a login in "+Config.getGridName())
			     .spanCell(2)
			     .alignCell("center");
		}
	...
	protected boolean checkAuthenticationNeeded(@Nonnull final Method content) {
		...
		if (State.get().sessionId()==null) {
			...
			Page.page()
			    .addHeader("Set-Cookie","coagulateslsessionid="+State.get().sessionId()+"; HttpOnly; Path=/; Secure;");
		}
	...
	private void logon() {
		...
		if (!username.isBlank()&&!password.isBlank()) {
			...
			SL.log()
			  .warning("Attempt to authenticate with incorrect password as '"+username+"' from "+state.getClientIP());
			return; // bad password
		}
