...
public class SLAPIMapper extends URLMapper<Method> {
	...
	protected void executePage(final Method content) {
		...
		try {
			if ((url!=null&&url.digest())||(urlprefix!=null&&urlprefix.digest())) {
				checkDigest();
			}
		...
		if (!theirObjectKey.isEmpty()) {
			if (!theirObjectKey.equals(objectKey)) {
				throw new SystemRemoteFailureException(
						"Object key mismatch - headers generated "+objectKey+" but they think it's "+theirObjectKey);
			}
		}
	...
	protected int processOutput(final HttpResponse response,final Method content) {
		...
		try {
			if (State.get().jsonOutNullable()==null) {
				throw new SystemImplementationException(
						"No response set up by "+content.getDeclaringClass().getCanonicalName()+"."+content.getName());
			}
	...
	private String requireHeader(final HttpRequest req,final String header) {
		return getHeader(true,req,header);
	}
	
	private String optionalHeader(final HttpRequest req,final String header) {
		return getHeader(false,req,header);
	}
	
	private String getHeader(final boolean mandatory,final HttpRequest req,final String header) {
		final Header[] headerSet=req.getHeaders(header);
		if (headerSet.length==0) {
			if (!mandatory) {
				return null;
			}
			final SystemRemoteFailureException e=
					new SystemRemoteFailureException("Mandatory data was not supplied to SL API processor");
			SL.report("Missing mandatory header "+header,e,State.get());
			throw e;
		}
		if (headerSet.length>1) {
			final SystemRemoteFailureException e=
					new SystemRemoteFailureException("Too much mandatory data was supplied to SL API processor");
