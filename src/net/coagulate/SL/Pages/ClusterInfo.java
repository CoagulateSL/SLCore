...
public class ClusterInfo extends ContainerHandler {
    ...
    protected void run(State state, Page page) {
        ...
        page.paragraph("<b>Cons:</b> Management of large numbers of distributed resources becomes complicated ; inter region comms must be set up in second life and objects must parse and process this information");
        ...
        page.paragraph("Products remain roughly the same, self contained objects that run stand-alone, but utilise an internet host/database for reporting/control, and nothing more.");
        page.paragraph("<b>Pros:</b> All the reliability and simplicity of Generation Zero, without the management overheads.  Solution is resilient to server failure as it's only used for offloading data.");
        ...
        page.paragraph("This generation resolves the asynchronous replication issue, and cleans up some implementation details around replication (i.e. having to initialise or reinitialise replicas)");
        page.paragraph("At the heart of Generation Three is MariaDB with Galera clustering, this provides a synchronously replicating database, allowing data to be consistently written across all nodes, rather than the previous 'will apply when I can' approach of Generation Two");
        page.paragraph("This also means cluster wide locking can occur which resolves some issues around making sure things don't happen twice on different servers / at the same time with other conflicting processes");
        page.paragraph("Due to the nature of this synchronous replication, three servers are required.  With asynchronous, each server simply queues up commands, and the other plays them back as and when it can, with no guarantees of timing or ordering with its own transactions.  For safe synchronous replication all nodes must reach all other nodes in real time to co-ordinate - this creates issues with a net-split as neither node knows if its the live or dead one, and thus a 3 node solution is used, where any 2 or more nodes form the database cluster.  It is possible to reduce it to one node through controlled shutdown, or start a singular node in emergencies or for recovery, but the replication mechanism assures 'consensus' by reaching a majority (2) of the (3) nodes.");
        page.paragraph("On top of this database sits an apache server and a custom Java solution which exposes HTML content for user interactions, and various API endpoints for LSL, usually in JSON over HTTP (with timestamped digest authentication)");
        page.paragraph("These three nodes allow solving of the problems in Generation Two in not being able to co-ordinate tasks (e.g. billing, dont want to be billed twice for something) and ensure updates are visible across all nodes instantly, rather than 'some time soon after, hopefully'");
        page.paragraph("At this point I have migrated ALL my previous services over to the Generation Three cluster and retired the previous solutions ; it's roughly backwards compatible with generation two solutions (the possibility of transaction rollback would only highlight design flaws in the original code), and can even host Generation One services, though they don't particularly benefit from this level of design or resilience.  I've started to re-engineer and tightly integrate previous services to maximally unify everything under a single 'Third Generation SL Networked Solution' (or fourth, if you like)");
        page.paragraph("<b>Pros:</b> Resilient, consistent, synchronous database cluster with Object Orientated programming driving LSL frontends; able to solve all previous generations requirements, and improve on them");
        page.paragraph("<b>Cons:</b> Requires three servers.  Still requires some coding knowledge to avoid problems with parallelism (though even generation zero may not avoid this, it's inherent to multi user environments)");
    }
}
