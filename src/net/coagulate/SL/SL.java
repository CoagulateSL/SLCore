...
public class SL extends Thread {
	...
	private static void startup() {
		try {
			...
			if (!Config.getDeveloperEmail().isEmpty() && !Config.getDevelopment()) {
				MailTools.mail(
	...
	public static void reportString(final String header,@Nullable final Throwable t,@Nullable final String additional) {
		...
		try {
			if (t!=null) {
				...
				if (!Config.getDeveloperEmail().isBlank()) { MailTools.mail((Config.getDevelopment()?"Dev":"PROD")+" EX : "+header+" - "+t.getLocalizedMessage(),output); } else {
					t.printStackTrace();
				}
				return;
			}
			if (!Config.getDeveloperEmail().isBlank()) { MailTools.mail((Config.getDevelopment()?"Dev":"PROD")+" EX : "+header,output); } else {
