...
public abstract class SLModule {
	...
	protected final boolean nextRun(final String name,final int interval,final int variance) {
		if (!nextRuns.containsKey(name)) {
			nextRuns.put(name,UnixTime.getUnixTime());
		} // why am i doing unix time stuff here...
		if (UnixTime.getUnixTime()<nextRuns.get(name)) {
			return false;
		}
		int nextRun=nextRuns.get(name)+interval;
		if (variance>0) {
			nextRun+=ThreadLocalRandom.current().nextInt(variance*2)-variance;
		}
	...
	public String getBuildDateString() {
		return convertDate(getBuildDate());
	}
		...
		try {
			...
			if (currentVersion>requiredVersion) {
				...
				if (Config.getDevelopment()) {
					logger.warning("Schema for "+schemaName+" is too new "+currentVersion+" > "+requiredVersion+
					               " (current>required), continuing as in DEVELOPMENT mode.  No schema upgrade is executed.");
					return;
				} else {
					logger.config("Schema for "+schemaName+" is too new "+currentVersion+" > "+requiredVersion+
					              " (current>required)");
					throw new SystemImplementationException(
							"Schema version too new on "+schemaName+" - current #"+currentVersion+" > required #"+
							requiredVersion+", terminating as a safety measure");
				}
			}
			while (currentVersion!=requiredVersion) {
				logger.config("Schema "+currentVersion+" is not of required version "+requiredVersion+
				              ", calling schemaUpgrade");
				final int newVersion=schemaUpgrade(db,schemaName,currentVersion);
				if (newVersion==currentVersion) {
					throw new SystemImplementationException(
							"Schema upgrade failed ; requested upgrade from "+currentVersion+" and remained at "+
							newVersion+", the target is version "+requiredVersion);
				}
				logger.config(
						"Upgraded schema from "+currentVersion+" to "+newVersion+" (target is "+requiredVersion+")");
			...
			t.printStackTrace();
			logger.config("Schema upgrade FAILED for "+schemaName+" version "+requiredVersion+
			              ".  Terminating as a safety measure.  It is advised you keep these logs to diagnose and resolve the failed upgrade.  You will probably need to manually resolve this condition.");
	...
	public int getSchemaVersion(final DBConnection db,final String schemaName) {
		try {
			return SystemManagement.get(db,schemaName);
		} catch (final Throwable t) {
